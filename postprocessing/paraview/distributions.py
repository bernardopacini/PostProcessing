...
def compute_section_properties(coords, calculate_chord=True, calculate_twist=True, calculate_thickness=True):
    """
    ...
    calculate_chord : bool
        Flag to enable chord calculation.
    calculate_twist : bool
        Flag to enable twist calculation.
    calculate_thickness : bool
        Flag to enable thickness calculation.
    ...
    # Find coordinate furthest from TE and two neighbors
    # TODO: Adjust to use average of TE points, not just upper point.
    xTE = coords[0, :]
    max_dist = 0.0
    i_max_dist = -1
    for k in range(1, np.size(coords, 0)):
        dist = np.linalg.norm(coords[k, :2] - xTE)
        if dist > max_dist:
            max_dist = dist
            i_max_dist = k

    xLE_down = coords[i_max_dist - 1, :]
    xLE = coords[i_max_dist, :]
    xLE_up = coords[i_max_dist + 1, :]
    ...
    c, r = circle_from_3_points(xLE_down, xLE, xLE_up)

    # Find true LE
    def minFunc(theta, c, r, xTE):
        x = np.array([r * np.cos(theta[0]) + c[0], r * np.sin(theta[0]) + c[1]])
        return -np.linalg.norm(x - xTE)

    res = scipy.optimize.minimize(minFunc, x0=[np.pi / 2], args=(c, r, xTE[:2]), bounds=[(0, 2 * np.pi)], tol=1e-12)
    xLE_comp = np.array([r * np.cos(res.x[0]) + c[0], r * np.sin(res.x[0]) + c[1]])

    # Compute chord
    chord = np.linalg.norm(xTE - xLE_comp)

    # Compute twist
    twist = np.rad2deg(np.arctan2((xLE_comp[1] - xTE[1]), -(xLE_comp[0] - xTE[0])))

    if calculate_thickness:
        # Rotate airfoil
        R = np.array(
            [
                [np.cos(np.deg2rad(twist[j])), -np.sin(np.deg2rad(twist[j]))],
                [np.sin(np.deg2rad(twist[j])), np.cos(np.deg2rad(twist[j]))],
            ]
        )
        coords_disp = (R @ (coords[:, :2] - xLE_comp).T).T

        # Isolate Coordinates
        coords_top = np.flip(coords_disp[0:i_max_dist, :], axis=0)
        coords_bot = coords_disp[i_max_dist + 1 :, :][0:-4]  # UPDATE THESE TO FIND TE
...
# # TODO: Rotate coordinates into X-Y plane

# # Sort
# coordsX, coordsY, coordsZ, _, arclen = plotSlices.sortAirfoil(
#     coords[:, 0], coords[:, 1], coords[:, 2], np.zeros(len(arclen)), arclen
# )
# coords = np.stack((coordsX, coordsY, coordsZ), axis=1)
# # TODO: Rotate into X - Y frame

# # Compute Chord, Twist, and Thickness
# chord[j], twist[j], thickness[j] = compute_section_properties(coords)

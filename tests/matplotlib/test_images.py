...
def run_example_script(script_path, output_dir):
    ...
    subprocess.run("python {} {}".format(script_path, output_image), shell=True, check=True)
...
class TestMatplotlibExamples(unittest.TestCase):
    ...
    def test_example(self, script_path):
        ...
        try:
            self.assertTrue(
                images_are_similar(baseline_image, output_image), "Images differ for {}!".format(script_name)
            )
